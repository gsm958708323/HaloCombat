...
namespace Combat.Demos
{
    ...
    public static class SeasonTwoDemo
    {
        public static void Run()
        {
            ...
            Actor bolt = null;
            trace.AdvanceUntil("projectile-spawn", "生成 Homing Projectile", () =>
            {
                bolt = FindBolt();
                return bolt != null;
            ...
            trace.AdvanceUntil("g1-hitstop", "G1 命中近桩并请求顿帧", () =>
            {
            ...
            trace.AdvanceUntil("homing-recovery", "解冻后 Homing 继续追踪并命中", () =>
            {
            ...
            world.Deliver(
                ...
                player, null, pAttr.GetFinal(AttrId.Atk));
            Actor pet = null;
            trace.AdvanceUntil("summon-spawn", "创建玩家 Owner 的召唤物", () =>
            {
                pet = FindPet();
                return pet != null;
