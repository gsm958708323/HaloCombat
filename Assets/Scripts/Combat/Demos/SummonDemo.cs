...
namespace Combat.Demos
{
    ...
    public static class SummonDemo
    {
        public static void Run()
        {
            ...
            trace.Step("初始化 Owner、目标与召唤目录", () => $"{DemoTrace.Snapshot(owner)} target={DemoTrace.Snapshot(stake)}");
            ...
            trace.Check("创建召唤物并建立 Owner 关系", summon != null, "召唤物存在", $"召唤物存在={(summon != null)}",
                () => summon == null ? "无 summon" : DemoTrace.Snapshot(summon));
            trace.Check("Owner 关系写入 Summon 与 Blackboard", summon != null && summon.GetComp<SummonComp>().OwnerId == owner.Id &&
            ...
            trace.AdvanceFor("召唤物跟随 Owner", 0.05f, 40, () => DemoTrace.Snapshot(summon));
            ...
            trace.Check("召唤物跟随后保持在 Owner 附近", Math.Sqrt(followDx * followDx + followDz * followDz) <= 2.2f,
            ...
            trace.AdvanceUntil("召唤物通过自己的 BT/Timeline 攻击", () => stake.GetComp<AttributeSet>().GetBase(AttrId.Hp) < hp0 - 0.1f,
                0.02f, 80, () => $"targetHp={stake.GetComp<AttributeSet>().GetBase(AttrId.Hp).ToString("F1")} summonHit={summonHit} {DemoTrace.Snapshot(summon)}");
            trace.Check("召唤物攻击造成伤害且 Source 正确", stake.GetComp<AttributeSet>().GetBase(AttrId.Hp) < 100f && summonHit,
            ...
            trace.AdvanceFor("准备召唤物的 Projectile 与 AoE 后代", 0.02f, 1,
            ...
            trace.AdvanceFor("Owner 死亡并级联清理召唤物及后代", 0.02f, 1,
                () => $"summonActive={summon.IsActive} descendants={CountOwnedRuntime(world, summon.Id)}");
            trace.Check("Owner 死亡后清理召唤物及其后代", !summon.IsActive && !world.TryGetActor(summon.Id, out _) &&
            ...
            trace.AdvanceFor("清理后继续推进世界", 0.05f, 20,
                () => $"targetHp={stake.GetComp<AttributeSet>().GetBase(AttrId.Hp).ToString("F1")}");
            trace.Check("清理后目标不再受到召唤物伤害", stake.GetComp<AttributeSet>().GetBase(AttrId.Hp) == hp1,
