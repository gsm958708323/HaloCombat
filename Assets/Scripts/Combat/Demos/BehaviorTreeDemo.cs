...
namespace Combat.Demos
{
    ...
    public static class BehaviorTreeDemo
    {
        public static void Run()
        {
            ...
            trace.Step("初始化 AI Actor 与独立 Blackboard", () => $"{DemoTrace.Snapshot(enemy)} target={DemoTrace.Snapshot(stake)}");
            // AI Actor 不应拥有玩家输入路径；每个 Actor 还必须使用独立的行为树黑板。
            trace.Check("初始 Blackboard 没有目标且 Actor 无玩家输入路径", !enemyBt.Board.Target.IsValid &&
            ...
            trace.Check("行为树播放 G1 且目标HP下降", sawG1 && stake.GetComp<AttributeSet>().GetBase(AttrId.Hp) < 100f && aiHits > 0,
            ...
            trace.Check("Knockdown 停止行为树正在播放的技能", enemy.GetComp<StateMachineComp>().Current == ActivityId.Knockdown &&
            ...
            trace.AdvanceFor("倒地期间推进并观察行为树停机", 0.02f, 20,
                () => $"targetHp={stake.GetComp<AttributeSet>().GetBase(AttrId.Hp).ToString("F1")} {DemoTrace.Snapshot(enemy)}");
            trace.Check("倒地期间目标HP保持不变", stake.GetComp<AttributeSet>().GetBase(AttrId.Hp) == hp,
