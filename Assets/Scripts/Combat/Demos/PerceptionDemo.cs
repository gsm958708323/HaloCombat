...
namespace Combat.Demos
{
    ...
    public static class PerceptionDemo
    {
        public static void Run()
        {
            ...
            trace.Step("初始化感知范围、Forced 与 Blackboard", () => $"{DemoTrace.Snapshot(enemy)} player={DemoTrace.Snapshot(player)}");
            trace.AdvanceFor("范围外目标不触发感知", 0.02f, 1,
                () => $"Forced={perception.Forced} Board.Target={board.Target}");
            trace.Check("范围外不设置 Forced 且敌人 Blackboard 保持无目标", !perception.Forced.IsValid &&
            ...
            trace.Check("受到伤害后记录强制目标", perception.Forced == player.Id,
            ...
            trace.AdvanceUntil("目标进入范围并写入 Blackboard", () => board.Target == player.Id, 0.02f, 3,
                () => $"Forced={perception.Forced} Board.Target={board.Target}");
            trace.Check("进入范围只更新主敌人的 Blackboard", board.Target == player.Id && !bystander.GetComp<BehaviorTreeComp>().Board.Target.IsValid,
            ...
            trace.AdvanceUntil("目标离开范围并清空 Blackboard", () => !board.Target.IsValid, 0.02f, 3,
                () => $"Forced={perception.Forced} Board.Target={board.Target}");
            trace.Check("离开范围后清空 Blackboard 目标", !board.Target.IsValid,
                "Board.Target为空", $"Board.Target={board.Target}", () => DemoTrace.Snapshot(enemy));
            player.GetComp<TransformComp>().Position = new SimVec3(1f, 0f, 0f);
            trace.AdvanceUntil("目标重新进入范围并再次获得", () => board.Target == player.Id, 0.02f, 3,
                () => $"Forced={perception.Forced} Board.Target={board.Target}");
            trace.Check("重新进入范围后再次获得目标", board.Target == player.Id,
            ...
            trace.AdvanceUntil("多个敌人独立获取同一目标", () =>
            ...
            trace.Check("多个敌人分别写入独立 Blackboard", first.GetComp<BehaviorTreeComp>().Board.Target == player.Id &&
