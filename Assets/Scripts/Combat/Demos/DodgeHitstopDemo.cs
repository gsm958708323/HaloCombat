...
namespace Combat.Demos
{
    ...
    public static class DodgeHitstopDemo
    {
        public static void Run()
        {
            ...
            trace.Step("初始化闪避、近战与顿帧事件", () => $"{DemoTrace.Snapshot(attacker)} target={DemoTrace.Snapshot(target)}");
            // Dodge Timeline 同时负责位移和无敌帧；命中无敌目标不得造成伤害或顿帧。
            input.Push(Season2Tokens.Dodge);
            trace.AdvanceUntil("闪避启动并进入无敌窗口", () => director.CurrentSkill == SkillNodeId.Dodge &&
            ...
            trace.AdvanceUntil("闪避结束并恢复 Root", () => !attacker.GetComp<TagComp>().Has(CommonTags.Invincible) &&
                fsm.Current == ActivityId.Root, 0.02f, 25, () => DemoTrace.Snapshot(attacker));
            float dx = attacker.GetComp<TransformComp>().Position.X - x0;
            trace.Check("闪避位移完成且无敌窗口关闭", dx >= 1f && dx <= 1.4f,
            ...
            trace.Check("倒地期间阻止闪避输入", fsm.Current == ActivityId.Knockdown && director.CurrentSkill != SkillNodeId.Dodge,
                "Activity=Knockdown 且未播放 Dodge", $"Activity={fsm.Current} skill={director.CurrentSkill}",
                () => DemoTrace.Snapshot(attacker));
            input.Clear();
            trace.AdvanceUntil("倒地恢复后允许后续输入", () => fsm.Current == ActivityId.Root, 0.05f, 12,
                () => DemoTrace.Snapshot(attacker));
            trace.Check("倒地恢复到 Root", fsm.Current == ActivityId.Root, "Activity=Root", $"Activity={fsm.Current}",
                () => DemoTrace.Snapshot(attacker));
            ...
            trace.Check("近战伤害事件与顿帧事件配对", meleeHit,
            ...
            trace.Check("默认伤害不主动顿帧", damage == damageBeforeDefault + 1 && hitstops == hitstopBeforeDefault,
            ...
            trace.Check("生成可冻结的 Homing Projectile", bolt != null, "Projectile 存在", $"Projectile存在={(bolt != null)}",
