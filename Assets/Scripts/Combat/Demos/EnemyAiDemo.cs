...
namespace Combat.Demos
{
    ...
    public static class EnemyAiDemo
    {
        public static void Run()
        {
            ...
            trace.Step("初始化 melee_guard 策略与 Home", () => $"{DemoTrace.Snapshot(enemy)} player={DemoTrace.Snapshot(player)}");
            bool sawAttack = false;
            // 初始距离在攻击范围附近，行为树应完成选敌并播放 G1。
            trace.AdvanceUntil("Guard 接敌并播放 G1", () =>
            {
            ...
            trace.Check("Guard 播放 G1 且玩家HP下降", sawAttack && player.GetComp<AttributeSet>().GetBase(AttrId.Hp) < 100f,
            ...
            trace.Check("记录 Guard 的 Home 出生点", board.Home.X == 0f && board.Home.Z == 0f,
            ...
            trace.AdvanceFor("目标离开后 Guard 回到 Home", 0.02f, 150, () =>
            {
            ...
            trace.Check("目标离开后回到 Home 附近", minDistanceToHome <= 0.6f && enemy.GetComp<TransformComp>().Position.X < 3f,
            ...
            trace.AdvanceFor("Home 附近无目标时巡逻", 0.05f, 10,
            ...
            trace.Check("Home 附近无目标时产生巡逻位移", Math.Sqrt(movedX * movedX + movedZ * movedZ) > 0.3f,
            ...
            trace.AdvanceUntil("目标重新出现后再次接敌", () =>
            {
            ...
            trace.Check("目标重新出现后恢复接敌流程", reacquired, "重新获得目标或播放 G1", $"reacquired={reacquired}",
                () => DemoTrace.Snapshot(enemy));
            ...
            trace.AdvanceFor("Knockdown 后停止移动和攻击", 0.02f, 5,
                () => DemoTrace.Snapshot(enemy));
            trace.Check("倒地后停止移动和攻击", enemy.GetComp<StateMachineComp>().Current == ActivityId.Knockdown &&
