...
namespace Combat.Demos
{
    ...
    public static class KnockdownDemo
    {
        public static void Run()
        {
            ...
            trace.Step("init", "初始化攻击者、目标与 Knockdown 状态机", () => DemoTrace.Snapshot(target));

            // 目标进入真实 Knockdown Activity，阻止攻击；重复施加只刷新计时器，
            // 不会重复累加 Downed Tag。
            world.Deliver(new IEffect[] { new KnockdownEffect { Duration = 0.5f } }, attacker, target, 10f);
            trace.Check("enter", "目标进入 Knockdown 并获得 Downed Tag", targetFsm.Current == ActivityId.Knockdown && targetTags.Has(CommonTags.Downed),
            ...
            trace.Check("attack-gate", "倒地期间阻止 Attack", !downedAttack, "TryEnter Attack=false", $"TryEnter Attack={downedAttack}",
                () => DemoTrace.Snapshot(target));
            int downedStacks = targetTags.Stack(CommonTags.Downed);
            world.Deliver(new IEffect[] { new KnockdownEffect { Duration = 0.5f } }, attacker, target, 10f);
            trace.Check("refresh", "重复 Knockdown 只刷新计时", targetFsm.Current == ActivityId.Knockdown && targetTags.Stack(CommonTags.Downed) == downedStacks,
            ...
            trace.Check("priority", "HitStun 不能覆盖 Knockdown", targetFsm.Current == ActivityId.Knockdown && !targetTags.Has(CommonTags.Stunned),
                "Activity=Knockdown 且 Stunned=false", $"Activity={targetFsm.Current} Stunned={targetTags.Has(CommonTags.Stunned)}",
                () => DemoTrace.Snapshot(target));
            trace.AdvanceUntil("recover", "等待 Knockdown 恢复 Root", () => targetFsm.Current == ActivityId.Root, 0.05f, 20,
                () => DemoTrace.Snapshot(target));
            trace.Check("recover-result", "恢复后清理 Downed Tag", targetFsm.Current == ActivityId.Root && !targetTags.Has(CommonTags.Downed),
            ...
            trace.Check("covers-hit", "Knockdown 覆盖已有 Hit", targetFsm.Current == ActivityId.Knockdown && !targetTags.Has(CommonTags.Stunned),
                "Activity=Knockdown 且 Stunned=false", $"Activity={targetFsm.Current} Stunned={targetTags.Has(CommonTags.Stunned)}",
                () => DemoTrace.Snapshot(target));
            trace.AdvanceFor("second-recover", "推进第二次倒地计时", 0.05f, 16, () => DemoTrace.Snapshot(target));
            ...
            trace.Check("skill-start", "倒地前技能已启动", attacker.GetComp<SkillDirectorComp>().IsPlaying, "技能播放中", $"playing={attacker.GetComp<SkillDirectorComp>().IsPlaying}",
            ...
            trace.Check("player-gate", "玩家倒地停止技能并清空输入", fsm.Current == ActivityId.Knockdown && tags.Has(CommonTags.Downed) &&
                ...
                () => DemoTrace.Snapshot(attacker));
            trace.AdvanceFor("player-recover", "推进玩家倒地计时", 0.05f, 16, () => DemoTrace.Snapshot(attacker));
            ...
            trace.Check("dead-priority", "Dead 覆盖 Knockdown 并成为终态", fsm.Current == ActivityId.Dead, "Activity=Dead", $"Activity={fsm.Current}",
                () => DemoTrace.Snapshot(attacker));
