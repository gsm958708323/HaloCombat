...
namespace Combat.Unity.Presentation
{
    public static class ProceduralVfxFactory
    {
        public static GameObject Create(Transform root, string key, Vector3 pos)
        {
            ...
            go.transform.SetParent(root, false); go.transform.position = pos;
            ...
            if (lower.Contains("slash") || lower.Contains("g1") || lower.Contains("g2"))
            {
                ...
                arc.transform.SetParent(go.transform, false); arc.transform.localScale = new Vector3(1.15f, .025f, 1.15f);
            ...
            else
            {
                ...
                ring.transform.SetParent(go.transform, false); ring.transform.localScale = Vector3.one * .35f;
                ring.GetComponent<Renderer>().material.color = lower.Contains("ground") ? new Color(1f, .35f, .08f) : new Color(1f, .9f, .45f);
        ...
        public static GameObject CreateLoop(Transform root, int visualId)
        {
            ...
            ring.transform.SetParent(go.transform, false); ring.transform.localPosition = new Vector3(0, .05f, 0);
